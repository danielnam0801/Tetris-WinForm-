...
namespace Tetris_10404남준성
{
    internal class Board
    {
        ...
        internal void Store(int bn, int tn, int x, int y)
        {
            for(int xx = 0; xx < 4; xx++)
            {
                for (int yy = 0; yy < 4; yy++)
                {
                    ...
                    {
                        board[x + xx, y + yy] += BlockValue.bValues[bn, tn, xx, yy];
                    }
                }
            }
            CheckLines(y + 3);
        ...
        private bool CheckLine(int y)
        {
            for(int xx = 0; xx<GameRule.BX; xx++)
            {
                if(board[xx, y] == 0 ) return false;
            }
            return true;
        }

        internal void ClearBoard() // clear all element in board;
        {
            for(int xx = 0; xx<GameRule.BX; xx++)
            {
